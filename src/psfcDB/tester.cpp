...
bool testFindNodesHost(PsfcIndex* index) {
    ...
    vector<Node*> nodes = index->findNodes_Host(make_tuple(MAX_LAT, -MAX_LONG),
...
bool testFindNodesHostNamed(PsfcIndex* index) {
    auto start = chrono::high_resolution_clock::now();
    vector<Node*> nodes = index->findNodesNamed_Host(
        make_tuple(MAX_LAT, -MAX_LONG), make_tuple(-MAX_LAT + 1, MAX_LONG - 1), "middle");
    auto end = chrono::high_resolution_clock::now();
    // Calculate the duration
    auto duration = chrono::duration_cast<chrono::microseconds>(end - start).count();
...
int main() {
    ...
    populateNodes(index, PAGE_SIZE * 40 * 10);

    testFindNodesHost(index);
    testFindNodesHostNamed(index);
}